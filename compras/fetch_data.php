...
if($sgbd == 'mysql'){
    $results = $pdo->prepare("SELECT id, produto_id, quantidade, preco, data FROM compras ORDER BY id LIMIT $start, $row_limit");
}elseif($sgbd == 'pgsql'){
    $results = $pdo->prepare("SELECT id, produto_id, quantidade, preco, data FROM compras ORDER BY id LIMIT $row_limit OFFSET $start");    
}
$results->execute();
